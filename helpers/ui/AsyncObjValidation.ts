...
import { WaitUntilOptions, Element } from 'webdriverio';
...
class ObjectValidation {
  ...
  async waitForElement(path, maxWait: Partial<WaitUntilOptions> = { timeout: 20000 }) {
    let element = null;
  ...
  async waitForSpinner(maxTime: number = 10000, additionalPause = 700) {
    let spinner: Element = await $(".MuiCircularProgress-svg");
    const options: WaitUntilOptions = {
      timeout: maxTime,
      timeoutMsg: `expected loading spinner to be gone after ${timeout} milliseconds`,
    };
